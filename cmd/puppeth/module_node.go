...
{{end}}
RUN \
  echo 'grbd --cache 512 init /genesis.json' > grbd.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.rubidium/keystore/ && cp /signer.json /root/.rubidium/keystore/' >> grbd.sh && \{{end}}
	echo $'exec grbd --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> grbd.sh

ENTRYPOINT ["/bin/sh", "grbd.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.rubidium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.rbdhash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.rubidium"],
		ethashdir:  infos.volumes["/root/.rbdhash"],
		port:       port,
