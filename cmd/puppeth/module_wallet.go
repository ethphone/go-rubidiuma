...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'grbd --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec grbd --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ethstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.RPCPort}}:7764"{{if not .VHost}}
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.rubidium
    environment:
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	rpcPort := infos.portmap["7764/tcp"]
