...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Rubidium app in browser mode", w.failure
	}
	if w.offline() {
		return "Rubidium app offline", w.failure
	}
	return fmt.Sprintf("Rubidium app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}
