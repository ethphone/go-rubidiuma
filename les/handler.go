...
import (
	...
	"github.com/rubidiumchain/go-rubidiuma/core/state"
	...
	"github.com/rubidiumchain/go-rubidiuma/trie"
)
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Rubidium protocol")
	...
	log.Info("Light Rubidium protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Rubidium peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Rubidium handshake failed", "err", err)
	...
	if err := pm.peers.Register(p); err != nil {
		clientErrorMeter.Mark(1)
		p.Log().Error("Light Rubidium peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Rubidium message handling failed", "err", err)
			if p.fcServer != nil {
				p.fcServer.DumpLogs()
			}
			return err
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Rubidium message arrived", "code", msg.Code, "bytes", msg.Size)
